    ...
    var RichText = function (_Group) {
        ...
        _createClass(RichText, [{
            ...
            value: function __inputText(text) {
                var p = this.$RichText;
                this.htmlText = p[1].slice(0, p[42]) + this.__changeText(text) + p[1].slice(p[42], p[1].length);
                p[41] += text.length;
            ...
            value: function __onKeyDown(e) {
                ...
                if (e.keyCode == 229) {
                    if (!p[31]) {
                        ...
                        if (p[8] == this.__input.$getNativeText()) {
                            this.__inputText(p[8]);
                            this.__input.$setNativeText("");
                            this.$RichText[7] = false;
                            p[31] = false;
                            p[30] == "";
                    ...
                    if (e.keyCode == 37 && p[41] == 0 || e.keyCode == 39 && p[41] == p[4]) {
                        return;
                    }
                    if (e.keyCode == 37) {
                        p[41]--;
            ...
            key: "$setHtmlText",
            value: function $setHtmlText(text, decodeHtml) {
                ...
                for (var i = 0, len = text.length; i < len; i++) {
                    ...
                    if (char == "<" && decodeHtml) {
                        last = i;
                    } else if (char == ">" && decodeHtml && last != -1) {
                        //找到 <...>  结构
                        //1. 分析标签名称
            ...
            value: function $decodeTextDisplay(line, text, font, htmlText, htmlStart) {
            ...
            key: "$addImage",
            value: function $addImage(line, attributes, font, htmlText) {
                var p = this.$RichText;
                var item;
                var bitmap;
                var displayLine = line.displayLines[line.displayLines.length - 1];
