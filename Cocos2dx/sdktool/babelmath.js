...
function compressComplete() {
    new ShellCommand("babel", ["./srcMath", "-d", "./src"], function () {
...
//生成 js 依赖关系的文件
            ...
            for (var i = 0; i < files.length; i++) {
                ...
                if (files[i].url.slice(0, "./srcMath/flower/".length) == "./srcMath/flower/") {
                    continue;
                }
            ...
            while (list.length) {
                ...
                for (var i = 0; i < list.length; i++) {
                    ...
                    if (find) {
                        list.splice(i, 1);
                        info.url = StringDo.replaceString(info.url, "srcMath", "src");
...
fileContent += "})(Math);\n";
file = new File("srcMath/Math2D.js");
fileContent = StringDo.replaceString(fileContent, "exports.", "math2d.");
//fileContent += "for(var key in math) {\n";
//fileContent += "\tflower[key] = math[key];\n";
//fileContent += "}\n";
